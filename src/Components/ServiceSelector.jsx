...
const ServiceSelector = ({ searchPreSelection }) => {
              ...
              {openService === service && isAvailable && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 space-y-2 z-10">
                  {services[service].map((sub, index) => {
                    ...
                    const isHighlighted = highlightedService === sub.name;

                    return (
                      <div
                        key={index}
                        onClick={() => handleSelect(service, sub)}
                        className={`px-4 py-2 rounded-lg cursor-pointer text-sm font-medium border flex justify-between transition-all ${
                          isSelected
